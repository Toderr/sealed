...
// The invite screen, shared by both link formats:
//   /invite/{base64}  — legacy long links (payload decoded from the URL)
//   /i/{code}         — short links (payload resolved from the DB)
//
// Both routes end up with the same InvitePayload, so everything below — the
// inviter header, deal preview, accept flow — is identical between them. The
// only difference is how the payload was obtained, which is the page's job.
//
// Note that this screen has ALWAYS re-fetched the deal from the DB and preferred
// that data over the payload (see the accept flow below). The payload is really
// just a seed for the first paint, which is exactly why the short link can drop
// it from the URL without losing anything.
...
import { type InvitePayload, useProfileStore } from "@/lib/profile-store";
...
export function InviteView({ payload }: { payload: InvitePayload }) {
...
/** Shown for a malformed base64 token, an unknown short code, or a deleted deal. */
export function InviteError({
  title = "Invalid invite link",
  detail = "This link may have expired or been modified.",
}: {
  title?: string;
  detail?: string;
}) {
    ...
    <InviteShell>
      <div className="flex flex-col items-center justify-center py-24 gap-4 text-center">
        ...
        <div>
          ...
          <p className="text-[13px] text-muted mt-1">{detail}</p>
        </div>
...
/** Loading state while a short code is resolved server-side. */
